...
import { Wallet, ArrowUpRight, ArrowDownRight, Trophy, X, AlertCircle, CheckCircle, Clock, ShoppingCart } from "lucide-react";
...
const typeConfig: Record<string, { label: string; icon: typeof ArrowUpRight; color: string }> = {
  ...
  saque:      { label: "Saque",      icon: ArrowDownRight, color: "text-danger"  },
  withdrawal: { label: "Saque",      icon: ArrowDownRight, color: "text-danger"  },
  aposta:     { label: "Aposta",     icon: ShoppingCart,   color: "text-warning" },
  buy:        { label: "Aposta",     icon: ShoppingCart,   color: "text-warning" },
  ganho:      { label: "Ganho",      icon: Trophy,         color: "text-success" },
  payout:     { label: "Ganho",      icon: Trophy,         color: "text-success" },
  sell:       { label: "Venda",      icon: ArrowUpRight,   color: "text-success" },
};

const fmt = (v: number) => isNaN(v) ? "0,00" : v.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  ...
  const handleSolicitarSaque = async () => {
    ...
    try {
      ...
      await supabase.from("transações").insert({
        user_id: user!.id, tipo: "saque", valor: valorNum, status: "pending",
    ...
    <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md shadow-xl">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {TIPOS_PIX.map(t => (
                    <button key={t.value} onClick={() => setTipoChave(t.value)}
                      className={`py-2 px-3 rounded-lg border text-sm font-medium transition-all ${tipoChave === t.value ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/50"}`}>
                      {t.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <Input type="text" placeholder="Digite sua chave Pix" value={chavePix} onChange={e => setChavePix(e.target.value)} className="bg-background border-border h-11" />
              </div>
              <div className="bg-warning/10 border border-warning/30 rounded-lg p-3 flex gap-2">
                <AlertCircle className="h-4 w-4 text-warning shrink-0 mt-0.5" />
                <p className="text-xs text-warning">Saques são processados manualmente em até 24 horas úteis.</p>
              </div>
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-border bg-card overflow-hidden divide-y divide-border/40">
            {transactions.map(tx => {
              const tipo = tx.tipo || "deposito";
              const cfg  = typeConfig[tipo] || typeConfig.deposito;
              const Icon = cfg.icon;
              const val  = Number(tx.valor) || 0;
              const isPositive = ["deposito","deposit","ganho","payout","sell"].includes(tipo);
              const descricao = tx.descricao || tx.mercado_nome
                ? `${tx.mercado_nome || ""}${tx.opcao ? ` — ${tx.opcao}` : ""}`
                : null;
